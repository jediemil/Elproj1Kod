...
void setLED(uint8_t red_percent, uint8_t green_percent, uint8_t blue_percent) {
    ...
    setPWM(htim, LED_R_CHANNEL, red_percent);    			// Red on Channel 2
    setPWM(htim, LED_G_CHANNEL, green_percent);  			// Green on Channel 3
    setPWM(htim, LED_B_CHANNEL, blue_percent);   			// Blue on Channel 1
}

void setLED(uint8_t led_percent, uint8_t tim_channel) {
	setPWM(&htim1, tim_channel, led_percent);
}
...
uint32_t readADC(ADC_HandleTypeDef* hadc, uint32_t channel) {
    ...
    sConfig.Rank = ADC_REGULAR_RANK_1;     // Set to rank 1 in the regular sequence
    sConfig.SamplingTime = ADC_SAMPLETIME_15CYCLES;  // You can adjust this as needed
    ...
    uint32_t value = HAL_ADC_GetValue(hadc);      // Get the result
